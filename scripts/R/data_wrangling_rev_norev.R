...
# Last Modified Date: Sat May 11 05:33:44 2024
...
d_rev <- rio::import(here::here("data", "prep", "prl", "groundhog_raw.csv"))
d_rev$is_reversal <- 1

d_norev <- rio::import(
  here::here("data", "prep", "prl", "groundhog_norev_raw.csv")
)
d_norev$is_reversal <- 0
...
# Remove rows with NA in column user_id
df_clean <- df_clean %>% drop_na(user_id)
