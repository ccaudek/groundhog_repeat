...
# Last Modified Date: Sat May 11 05:07:58 2024
#
# 👉 Note that it is necessary to refer to "Task2noRev_n_1".
...
subj_codes <- rio::import(
  here::here("data", "raw", "control_data", "data.xlsx")
) |>
  dplyr::select(
    subj_code_1, Task2noRev_n_1,
    context_1, control_1,
    post_context_1, coin, TIME_start,
    TIME_end, TIME_total
  )

subj_codes <- subj_codes[!is.na(subj_codes$Task2noRev_n_1), ]
subj_codes$subj_idx <- subj_codes$Task2noRev_n_1
subj_codes$Task2noRev_1 <- NULL


# Import no-reversal PRL data form multiple files ------------------------------

dir <- here::here("data", "raw", "control_data")


# Filenames for sessions without reversal:
file_names <-
  as.character(list.files(path = dir, pattern = "prl_EMA_norev"))
...
print("Saving output")
rio::export(d, here::here("data", "prep", "prl", "groundhog_norev_raw.csv"))
